    ...
    <div class="bg-white h-full w-full">
      <div class="flex flex-row items-center justify-between flex-nowrap h-24 bg-gray-100 p-5 rounded-lg">
        <div class="w-2/3 flex-auto mr-4">
          <label class="relative block w-full">
            ...
            <input class="placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-12 pr-3 shadow-inner focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Search for organizations, trading amounts..." type="text" name="search"/>
          </label>
        </div>
        <div class="w-1/3 flex flex-nowrap flex-row items-center">
          <div>
            <button class="py-2 px-4 button--shadow-inner text-base capitalize font-semibold bg-white rounded-md text-blue-500 hover:bg-blue-500 hover:text-white transition-all whitespace-nowrap" @click="filtersVisible = !filtersVisible">
              <i class="fa-solid fa-filter"></i>
              Filters
            </button>
          </div>
          <div class="mx-6 relative">
            <button class="relative z-20 py-2 px-4 button--shadow-inner drop-shadow text-base capitalize font-semibold bg-white rounded-md whitespace-nowrap hover:bg-blue-500 hover:text-white transition-all text-blue-500" @click="menuVisible= !menuVisible" type="button" :class="{'active_btn': menuVisible}">
      ...
      <div class="flex flex-row justify-between items-center p-5 bg-gray-100 rounded-lg mb-10">
        <div class="w-3/5 text-xs text-gray-500 font-semibold">
          <p>1-<span>{{  }}</span> of {{ entries.length }}</p>
        </div>
      ...
      <Filters :showEntries="showEntries" :currentEntries="currentEntries" @changeEntries="changingEntries($event)" :columns="columns" @closeFilters="closeFilterModal($event)" :filtersVisible="filtersVisible"/>
...
// const tradesData = require('@/data/data.json');
...
export default {
  ...
  methods: {
    // async getAllData() {
    //   const res = await fetch('~/static/data.json', { mode: 'no-cors'})
    //   return res.json()
    // },
